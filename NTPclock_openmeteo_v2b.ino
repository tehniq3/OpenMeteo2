/* Nicu FLORICA (niq_ro) used AI to obtain this sketch
...
void setup() {
  ...
  // --- FIX PENTRU ARDUINO 1.8.19 SI ESP32 CORE 2.0.5 ---
...
void loop() {
  ...
  if (millis() - ultimulSchimb >= intervalSchimb) {
    ...
    // MODIFICARE: Sarim peste UV DOAR daca valoarea este 0 (nu ne mai bazam pe ore)
    if (ecranCurent == 4 && uvIndex <= 0.0) {
      ecranCurent = 5; 
    }
...
void obtineTextVremeLung(int cod, char* outText) {
  switch(cod) {
    case 0:   strcpy(outText, "   Cer senin   "); break; // MODIFICAT AICI
